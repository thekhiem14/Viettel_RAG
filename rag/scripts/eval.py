...
def _diff_body(pred_body: dict, gt_body: dict) -> dict:
    """So sánh từng param. Strip whitespace key trước khi so sánh (GT có thể có key 'isAllCustomer ').

    Returns dict {missing, extra, wrong, correct}.
    """
    pred_body = {k.strip(): v for k, v in pred_body.items()}
    gt_body = {k.strip(): v for k, v in gt_body.items()}
...
def main() -> None:
    ...
    for r in results:
        ...
        if pred_code == "call_api":
            ...
            try:
                ...
                # Param-level diff (chỉ chấm khi path đúng)
                if pred.get("path") == gt_api.get("path"):
    ...
    with open(out_api_diff, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["id", "path", "n_correct", "n_total", "missing", "extra", "wrong"],
