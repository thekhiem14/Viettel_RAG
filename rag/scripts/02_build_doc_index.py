...
Prerequisites:
    - Markdown files đã convert sẵn ở config.DOC_MD_DIR (flat: Public_XXX.md)
"""
...
def main(force: bool = False) -> None:
    ...
    print(f"[02_build_doc] chunked {len(chunks)} chunks from {config.DOC_MD_DIR}")
    if not chunks:
        raise RuntimeError(f"No chunks produced. Check DOC_MD_DIR={config.DOC_MD_DIR}")
